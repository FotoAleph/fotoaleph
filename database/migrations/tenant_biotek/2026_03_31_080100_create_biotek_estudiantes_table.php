...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::connection('tenant_biotek')->hasTable('estudiantes')) {
            Schema::connection('tenant_biotek')->create('estudiantes', function (Blueprint $table) {
                ...
                $table->string('sesion_uno')->nullable();
                $table->string('sesion_dos')->nullable();
                $table->timestamps();
            });
        }
        
        if (! Schema::connection('tenant_biotek')->hasTable('estudiantes_multimedia')) {
            Schema::connection('tenant_biotek')->create('estudiantes_multimedia', function (Blueprint $table) {
                $table->id();
    ...
    {
        Schema::connection('tenant_biotek')->dropIfExists('estudiantes_multimedia');
