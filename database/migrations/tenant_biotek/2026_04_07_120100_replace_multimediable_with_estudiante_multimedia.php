...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $targetHasRows = DB::connection('tenant_biotek')->table('estudiante_multimedia')->exists();

        if (! $targetHasRows) {
            $rows = DB::connection('tenant_biotek')
                ->table('multimediable')
            ...
            if ($rows !== []) {
                DB::connection('tenant_biotek')->table('estudiante_multimedia')->insert($rows);
            }
        }

        Schema::connection('tenant_biotek')->drop('multimediable');
    }
    ...
    public function down(): void
    {
        ...
        $legacyHasRows = DB::connection('tenant_biotek')->table('multimediable')->exists();
        ...
        if (! $legacyHasRows) {
            $rows = DB::connection('tenant_biotek')
                ...
                ->get()
                ->map(fn ($row) => [
                    'multimediable_type' => App\Models\BiotekEstudiante::class,
            ...
            if ($rows !== []) {
                DB::connection('tenant_biotek')->table('multimediable')->insert($rows);
            }
        }

        Schema::connection('tenant_biotek')->drop('estudiante_multimedia');
    }
};